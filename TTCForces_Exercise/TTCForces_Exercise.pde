...
float k_goal = 15;  //TODO: Tune this parameter to agent stop naturally on their goals
float k_avoid = 100;
float agentRad = 40;
float goalSpeed = 100;
...
void setup(){
  ...
  agentPos[0] = new Vec2(450,75);
  agentPos[1] = new Vec2(300,500);
  agentPos[2] = new Vec2(220,220);
  agentPos[3] = new Vec2(650,220);
  agentPos[4] = new Vec2(600,500);
  goalPos[0] = new Vec2(220,220);
  goalPos[1] = new Vec2(650,220);
  goalPos[2] = new Vec2(600,500);
  goalPos[3] = new Vec2(300,500);
  goalPos[4] = new Vec2(450,75);
...
Vec2 computeAgentForces(int id){
  ...
  for (int j = 0; j < numAgents; j++) {
    if (id != j) {
      if (agentPos[j].distanceTo(goalPos[j]) < 5) {
        continue;
      }
      Vec2 A_current = agentPos[id];
      Vec2 A_vel = agentVel[id];
      Vec2 B_current = agentPos[j];
      Vec2 B_vel = agentVel[j];
      float ttc = computeTTC(A_current, A_vel, agentRad, B_current, B_vel, agentRad);
      Vec2 A_future = A_current.plus(A_vel.times(ttc));
      Vec2 B_future = B_current.plus(B_vel.times(ttc));
      Vec2 rel = A_future.minus(B_future).normalized();
      acc.add(rel.times(k_avoid * (1/ttc)));
    }
  }
  return acc;
}
...
void draw(){
  ...
  fill(20,200,150);
  for (int i = 0; i < numAgents; i++){
    circle(agentPos[i].x, agentPos[i].y, agentRad*1.5);
  }
}

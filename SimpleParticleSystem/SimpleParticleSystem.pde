...
void update(float dt) {
  ...
  int speedMultiplier = shiftPressed ? 2 : 1;
  float speed = obstacleSpeed * speedMultiplier;
  if (leftPressed) obstacleVel.add(new Vec2(-1, 0));
  if (rightPressed) obstacleVel.add(new Vec2(1, 0));
  if (upPressed) obstacleVel.add(new Vec2(0, -1));
  if (downPressed) obstacleVel.add(new Vec2(0, 1));
  if (obstacleVel.length() > 0) {
    obstacleVel.normalize();
  }
  obstacleVel.mul(dt * speed);
  spherePos.add(obstacleVel);
  ...
  for (int i = 0; i <  numParticles; i++) {
    ...
    if (pos[i].distanceTo(spherePos) < (sphereRadius+r)) {
      ...
      vel[i].add(projAB(obstacleVel.times(80), normal));
    }
  }
}
